...
plot(Al2O3~BaO, RBPottery, pch=as.numeric(Region)-1)
...
plot(RD~H, BACups, pch=as.numeric(Phase)-1)
legend("topright", levels(BACups$Phase), title="Phase", pch=0:2)
...
table(OxfordPots$OxfordPct, OxfordPots$WaterTrans)
# Pearson's Chi-squared test
chisq.test(EWBurials$Goods, EWBurials$Group, correct=FALSE)
...
table(graveyards$Letter.styles, graveyards$Date.of.monument)
# Pearson's Chi-squared test
chisq.test(graveyards$Letter.styles, graveyards$Date.of.monument, correct=FALSE)

(graveyards.tbl <- xtabs(~Letter.styles+Date.of.monument, graveyards))

barplot(graveyards.tbl,main="Letter Style vs Date",ylab="Style Code",xlab="Date",las=2)
